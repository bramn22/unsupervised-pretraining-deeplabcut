...
def create_training_dataset(config,num_shuffles=1,Shuffles=None,windows2linux=False):
    ...
    path_train_config = str(os.path.join(cfg['project_path'],Path(modelfoldername),'train','pretrain_pose_cfg.yaml'))
    path_test_config = str(os.path.join(cfg['project_path'],Path(modelfoldername),'test','pretrain_pose_cfg.yaml'))
    ...
    defaultconfigfile = str(Path(config).parents[0].parents[0] / 'pretrain_pose_cfg.yaml')
