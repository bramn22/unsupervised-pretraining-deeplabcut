...
def create_new_project(project, experimenter, videos, pretrain_videos, working_directory=None, copy_videos=False,videotype='.avi'):
    ...
    from pretraining import auxiliaryfunctions as af
    date = dt.today()
    ...
    pretrain_path = project_path / 'pretrain'
    pretrain_data_path = pretrain_path / 'training-datasets'
    pretrain_video_path = project_path / 'pretrain-videos'
    for p in [video_path, data_path, shuffles_path, results_path, pretrain_path, pretrain_data_path, pretrain_video_path]:
        p.mkdir(parents=True, exist_ok=DEBUG)
        print('Created "{}"'.format(p))

    video_sets = handle_videos(videos, videotype, video_path, copy_videos, data_path)
    pretrain_video_sets = handle_videos(pretrain_videos, videotype, pretrain_video_path, copy_videos, pretrain_data_path)

    #        Set values to config file:
    cfg_file,ruamelFile = af.create_config_template()
    ...
    af.write_config(projconfigfile,cfg_file)

    print('Generated "{}"'.format(project_path / 'config.yaml'))
    #        Set values to config file:
    cfg_file, ruamelFile = af.create_pretrain_config_template()
    cfg_file['Task'] = project
    cfg_file['scorer'] = experimenter
    cfg_file['video_sets'] = pretrain_video_sets
    cfg_file['project_path'] = str(project_path)
    cfg_file['date'] = d
    cfg_file['cropping'] = False
    cfg_file['numframes2pick'] = 20
    ...
    projconfigfile_pretrain = os.path.join(str(project_path), 'pretrain_config.yaml')
    # Write dictionary to yaml  config file
    af.write_config(projconfigfile_pretrain, cfg_file, ruamelFile)

    print('Generated "{}"'.format(project_path / 'pretrain_config.yaml'))
    ...
    return projconfigfile, projconfigfile_pretrain


def handle_videos(videos, videotype, video_path, copy_videos, data_path=None):
