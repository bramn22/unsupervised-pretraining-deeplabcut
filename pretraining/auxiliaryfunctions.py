...
def create_pretrain_config_template():
    """
        Creates a template for pretrain_config.yaml file. This specific order is preserved while saving as yaml file.
        """
    import ruamel.yaml
    yaml_str = """\
    # Project definitions (do not edit)
...
def write_config(configname,cfg, ruamelFile=None):
    """
    Write structured config file.
    
    """
    with open(configname, 'w') as cf:
        if ruamelFile is not None:
            cfg_file = cfg
        else:
            ruamelFile = ruamel.yaml.YAML()
