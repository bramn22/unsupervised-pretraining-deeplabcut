...
def train_network(config,shuffle=1,trainingsetindex=0,gputouse=None,max_snapshots_to_keep=5,autotune=False,displayiters=None,saveiters=None,maxiters=None, modified_version=True):
    ...
    if modified_version:
        from modified.train import train
    else:
    ...
    if not poseconfigfile.is_file():
      ...
      try:
          train(cfg, str(poseconfigfile),displayiters,saveiters,maxiters,max_to_keep=max_snapshots_to_keep) #pass on path and file name for pose_cfg.yaml!
